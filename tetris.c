/**
 * @file tetris.c
 * @brief Реализация игры Tetris в виде конечного автомата.
 */
#include "tetris.h"
 ...
 *
 * Функция `game_loop` управляет игровым процессом, обрабатывая ввод пользователя,  
 * обновляя состояние игры и отображая интерфейс. Также она обрабатывает паузу и завершение игры.
 * 
 * @details 
 ...
 *    - Раз в (1000 / speed) мс вызывается автоматическое обновление для смещения фигуры вниз на 1 линию.
 *    - Отображается текущее состояние игры.
 *    - Если активирована пауза, ожидается её отключение или рестарта игры или выхода в главное меню.
 *    - Если игра окончена, отображается меню завершения и ожидается новая игра или выход.
