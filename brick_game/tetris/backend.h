 ...
 *
 * Выделяет память для игрового поля и фигур, создает игровое поле и генерирует первые фигуры.  
 * Устанавливает начальное состояние игры и загружает рекордный счет из файла.
 */
void init_condition();
 ...
 * сбрасывает очки и обновляет уровень. Если поле не было инициализировано, вызывает `init_condition()`.  
 * После инициализации игра переходит в состояние движения фигур.
 */
void start_game();
 ...
 * 
 * @return Указатель на структуру `condition_t`, содержащую текущее состояние игры.
 */
