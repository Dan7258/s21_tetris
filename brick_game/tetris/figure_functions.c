...
void s21_add_figure_on_field() {
  ...
  for (int m = 0; m < condition->figure->matrix->rows; m++) {
    for (int n = 0; n < condition->figure->matrix->columns; n++) {
      if (m + condition->figure->y >= 0 && m + condition->figure->y < ROWS_FIELD && 
...
int s21_is_figure_valid(figure_t *figure) {
  ...
  for(int m = figure->matrix->rows - 1; m > 0 && flag; m--) {
    for(int n = 0; n < figure->matrix->columns && flag; n++) {
      ...
      if(figure->matrix->matrix[m][n] && ((n + figure->x) < 0 || (n + figure->x) >= COLS_FIELD)) {
        flag = 0;
      }
      if(flag && figure->matrix->matrix[m][n] && (m + figure->y) >= ROWS_FIELD) {
        flag = 0;
      }
      if(flag && condition->field->matrix[m + figure->y][n + figure->x] && figure->matrix->matrix[m][n]) {
        flag = 0;
      }
    }
  }
...
void s21_fix_figure(figure_t *figure) {
  ...
  for(int m = figure->matrix->rows - 1; m > 0; m--) {
    for(int n = 0; n < figure->matrix->columns; n++) {
      if(figure->matrix->matrix[m][n] && (m + figure->y) >= ROWS_FIELD) {
        figure->y -= ((m + figure->y) - (ROWS_FIELD - 1));
      } else if(figure->matrix->matrix[m][n] && (n + figure->x) < 0) {
        figure->x += 0 - (n + figure->x);
      } else if(figure->matrix->matrix[m][n] && (n + figure->x) >= COLS_FIELD) {
        figure->x -= (n + figure->x) - (COLS_FIELD - 1);
      } else if(figure->matrix->matrix[m][n] && condition->field->matrix[m + figure->y][n + figure->x] && m <= figure->matrix->columns / 2) {
        for(;figure->matrix->matrix[m][n] && condition->field->matrix[m + figure->y][n + figure->x]; figure->x++) {
        }
      } else if(figure->matrix->matrix[m][n] && condition->field->matrix[m + figure->y][n + figure->x] && m > figure->matrix->columns / 2) {
        for(;figure->matrix->matrix[m][n] && condition->field->matrix[m + figure->y][n + figure->x]; figure->x--) {
        }
