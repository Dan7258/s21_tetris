 ...
 * @param hold Флаг зажатия кнопки (true - удержание, false - одиночное нажатие).
 * 
 * @details  
 * Алгоритм работы:
 * 1. Получаем текущее состояние игры (`condition`) с помощью `get_current_condition()`.
 * 2. В зависимости от переданного действия `action` выполняем соответствующие операции:
 *    - **Start**: Запуск игры (`start_game()`).
 *    - **Pause**: Если статус игры не `InitG`, переключаем между `PauseG` и `MovingG`.
 *    - **Terminate**:
 *      - Если статус `PauseG`, завершаем игру (`game_over()`).
 *      - Если статус `GameOverG`, очищаем данные (`clean_condition()`).
 *    - **Left**: Если статус `MovingG`, перемещаем фигуру влево (`move_left()`).
 *    - **Right**: Если статус `MovingG`, перемещаем фигуру вправо (`move_right()`).
 *    - **Down**:
 ...
 * @details  
 * Алгоритм работы:
 * 1. Получаем текущее состояние игры (`condition`) через `get_current_condition()`.
